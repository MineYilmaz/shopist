...
import com.shopist.pages.CommonPageElements;
...
import static com.shopist.utilities.Driver.get;
...
public class DynamicClick {
    ...
    public static List<WebElement> annotationFieldValueFinder() {
        ...
        if (!findBy.xpath().equals("")) {
            ...
            webElement = get().findElements(By.xpath(annotationValue));
        } else if (!findBy.css().equals("")) {
            annotationValue = findBy.css();
            webElement = get().findElements(By.cssSelector(annotationValue));
        } else if (!findBy.id().equals("")) {
            annotationValue = findBy.id();
            webElement = get().findElements(By.id(annotationValue));
        } else if (!findBy.linkText().equals("")) {
            annotationValue = findBy.linkText();
            webElement = get().findElements(By.linkText(annotationValue));
        } else if (!findBy.partialLinkText().equals("")) {
            annotationValue = findBy.partialLinkText();
            webElement = get().findElements(By.partialLinkText(annotationValue));
        } else if (!findBy.name().equals("")) {
            annotationValue = findBy.name();
            webElement = get().findElements(By.name(annotationValue));
        } else if (!findBy.className().equals("")) {
            annotationValue = findBy.className();
            webElement = get().findElements(By.className(annotationValue));
        } else if (!findBy.tagName().equals("")) {
            annotationValue = findBy.tagName();
            webElement = get().findElements(By.tagName(annotationValue));
        }
    ...
    public static WebElement[] annotationFieldValueFinderArray() {
        ...
        if (!findBy.xpath().equals("")) {
            annotationValue = findBy.xpath();
            List<WebElement> elementList = get().findElements(By.xpath(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        } else if (!findBy.css().equals("")) {
            annotationValue = findBy.css();
            List<WebElement> elementList = get().findElements(By.cssSelector(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        } else if (!findBy.id().equals("")) {
            annotationValue = findBy.id();
            List<WebElement> elementList = get().findElements(By.id(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        } else if (!findBy.linkText().equals("")) {
            annotationValue = findBy.linkText();
            List<WebElement> elementList = get().findElements(By.linkText(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        } else if (!findBy.partialLinkText().equals("")) {
            annotationValue = findBy.partialLinkText();
            List<WebElement> elementList = get().findElements(By.partialLinkText(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        } else if (!findBy.name().equals("")) {
            annotationValue = findBy.name();
            List<WebElement> elementList = get().findElements(By.name(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        } else if (!findBy.className().equals("")) {
            annotationValue = findBy.className();
            List<WebElement> elementList = get().findElements(By.className(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        } else if (!findBy.tagName().equals("")) {
            annotationValue = findBy.tagName();
            List<WebElement> elementList = get().findElements(By.tagName(annotationValue));
            webElement = elementList.toArray(new WebElement[elementList.size()]);
        }
